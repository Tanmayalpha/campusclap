...
import 'package:campusclap/Models/about_us_model.dart';
import 'package:campusclap/Services/api_services/http_services.dart';
...
class AboutUsView extends StatelessWidget {
      ...
      child: Column(
        children: [
          ...
          gradientContainerChild(
              aboutUs[index].sixTxtone.toString(), aboutUs[index].sixDeatilone.toString()),
          gradientContainerChild(
              aboutUs[index].sixTxttwo.toString(), aboutUs[index].sixDeatiltwo.toString()),
          gradientContainerChild(
              aboutUs[index].sixTxtthree.toString(), aboutUs[index].sixDeatilthree.toString()),
        ],
      ),
    );
  }
  ...
  Widget scaffoldBody(List<About>? aboutUs) {
          ...
          return Column(
            children: [
              ...
              if (aboutUs[index].oneEnable.toString() != "0")
                containerOneImage(
                    aboutUs[index].oneImage.toString(), aboutUs[index].oneHeading.toString()),
              if (aboutUs[index].oneEnable.toString() != "0")
                conGradient(aboutUs[index].oneText.toString(), colors.gradientColorsList),
              if (aboutUs[index].twoEnable.toString() != "0")
                heading(aboutUs[index].twoHeading.toString(), colors.primary),
              if (aboutUs[index].twoEnable.toString() != "0")
                simpleText(aboutUs[index].twoText.toString()),
              if (aboutUs[index].twoEnable.toString() != "0")
                showImage(aboutUs[index].twoImageone.toString()),
              if (aboutUs[index].twoEnable.toString() != "0")
                conGradient(aboutUs[index].twoTxtone.toString(), colors.gradientColorsList),
              if (aboutUs[index].twoEnable.toString() != "0")
                simpleText(aboutUs[index].twoImagetext.toString()),
              if (aboutUs[index].threeEnable.toString() != "0")
                heading(aboutUs[index].threeHeading.toString(), colors.primary),
              if (aboutUs[index].threeEnable.toString() != "0")
        ...
        return !snapshot.hasData
            ? Center(
                child: CircularProgressIndicator(
                  valueColor:
                      new AlwaysStoppedAnimation<Color>(colors.primary),
                ),
              )
            : Scaffold(
          appBar: PreferredSize(
              preferredSize: Size.fromHeight(80),
              child: commonAppBar(context, text: "AboutUs")),
                backgroundColor: colors.whiteTemp,
                body: scaffoldBody(snapshot.data),
              );
      },
    );
  }
