...
class CertificateExamScreen extends StatefulWidget {
  const CertificateExamScreen({Key? key, this.courseDetails, this.ispaid}) : super(key: key);
  ...
  Widget build(BuildContext context) {
    return  SafeArea(
      child: Scaffold(
        ...
        body: isLoading ? const Center( child: CircularProgressIndicator(color: colors.primary,),)
            : widget.courseDetails?.assessmentBookingSlotId == null ?

        slotList.isEmpty ? const Center(child: Text('No slot found')) : Column(
          children: [
            SizedBox(height: 10,),
            InkWell(
              onTap: (){
              ...
              child: Container(
                ...
                decoration:  BoxDecoration(
                    ...
                    boxShadow: const [
                      BoxShadow(spreadRadius: 0.5,blurRadius: 1,offset: Offset(1, 1), color: colors.darkColor),],
                    borderRadius: BorderRadius.circular(10),
                    border: Border.all(color: colors.darkColor)
                ),
                child:  Center(
                  child: selectedDate != null ? Text('${date}',textAlign: TextAlign.center,
                      style: const TextStyle(color: colors.textColor,fontSize: 20)): const Text('Select Date',textAlign: TextAlign.center,
          ...
          mainAxisSpacing: 2.0,
                ...
                itemBuilder: (context, index) {
                  ...
                  DateTime time = DateFormat('HH:mm:ss').parse(date.fromTime ?? '');
                  DateTime time2 = DateFormat('HH:mm:ss').parse(date.toTime ?? '');

                  String formaTime = DateFormat('h:mm a').format(time);
                  String toTime = DateFormat('h:mm a').format(time2);
                  return Card(
                      ...
                      child :InkWell(
                        ...
                        child: Container(
                          ...
                          decoration:  BoxDecoration(
                              color: date.isSelected ?? false ? colors.primary  : colors.whiteTemp,
                              boxShadow: const [
                                BoxShadow(spreadRadius: 0.5,blurRadius: 1,offset: Offset(1, 3), color: colors.darkColor),],
                              borderRadius: BorderRadius.circular(10),
                          ...
                          child: Row(
                            ...
                            children: [
                              Text(' ${formaTime}', style: TextStyle(color: date.isSelected ?? false ? colors.whiteTemp : colors.textColor),),
                              Text('To', style: TextStyle(color: date.isSelected ?? false ? colors.whiteTemp : colors.textColor),),
                              Text('${toTime}',style: TextStyle(color: date.isSelected ?? false ? colors.whiteTemp : colors.textColor))
                            ],),
                        ),
                      )
                  );
           ...
           btnLoading ? const Center( child: CircularProgressIndicator(color: colors.primary,),) :
           ComenBtn(
              title: 'Book Selected Slots', onPress: (){
              ...
              if(slotId !=null) {
                ...
                Fluttertoast.showToast(msg: 'select slot first');
              }
...
List <List<SlotData>> finalSlotList = [] ;
  ...
  Future<void> getBookingSlots() async {
    ...
    http.Response res = await http.post(
      Uri.parse("${APIData.getSlotes}${APIData.secretKey}"),
      headers: {
        "Accept": "application/json",
        "Authorization": "Bearer $authToken",
      },
      body: {
        'course_id': '40',//id/*widget.courseDetails?.course?.id.toString()*/
      }
    );
  ...
  Future<void> bookingSlots() async {
    ...
    http.Response res = await http.post(
        Uri.parse("${APIData.bookSlot}${APIData.secretKey}"),
        headers: {
          "Accept": "application/json",
          "Authorization": "Bearer $authToken",
        },
  ...
  Widget assessmentWidget() {
    print('${assessmentResponse?.data?.bookedslotes?.date} ${assessmentResponse?.data?.bookedslotes?.fromTime}');
    return Padding(
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              _buildDetail('Title', assessmentResponse?.data?.details?.title ?? ''),
              _buildDetail('Description', assessmentResponse?.data?.details?.description ?? ''),
              _buildDetail('Marks', assessmentResponse?.data?.details?.marks.toString() ?? ''),
              _buildDetail('Passing Marks', assessmentResponse?.data?.details?.passingMarks.toString() ?? ''),
              _buildDetail('Course Name', assessmentResponse?.data?.details?.coursename ?? ''),
              const SizedBox(height: 16),
              _buildHeading('Booked Slot Details'),
              _buildDetail('Slot ID', assessmentResponse?.data?.bookedslotes?.slotId.toString() ?? ''),
             // _buildDetail('User ID', bookedSlotDetails.userId.toString()),
              _buildDetail2('Booked on', assessmentResponse?.data?.bookedslotes?.createdAt ?? ''),
              _buildDetail('Booking', assessmentResponse?.data?.bookedslotes?.date ?? ''),
              const SizedBox(height: 16),

             DateTime.now().isAfter(DateTime.parse('${assessmentResponse?.data?.bookedslotes?.date} ${assessmentResponse?.data?.bookedslotes?.fromTime}')) ? Center(
                child: ComenBtn(title: 'Give Exam',onPress: (){
                  launchUrl(Uri.parse('${assessmentResponse?.data?.bookedslotes?.url}/${widget.courseDetails?.course?.id.toString()} '));
                },),
              ) : SizedBox(),
